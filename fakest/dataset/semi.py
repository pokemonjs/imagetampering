...
class SemiDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # print(1,np.max(np.array(img)),np.min(np.array(img))) # (0,255)
        # print(1,np.unique(np.array(mask))) # (0,1,255) 255是cut时padding的
        img, mask = normalize(img, mask)
        # print(2,np.max(np.array(img)),np.min(np.array(img))) # (-2.x,2.x)
        # print(2,np.unique(np.array(mask))) # (0,1,255)
        # 是否/255不影响normlize
