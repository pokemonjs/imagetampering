...
best_model = None
model_choices = ['deeplabv3plus', 'pspnet', 'deeplabv2']
model_index = 0
...
def parse_args():
    ...
    parser.add_argument('--backbone', type=str, choices=['resnet50', 'resnet101'], default=backbone)
    parser.add_argument('--model', type=str, choices=['deeplabv3plus', 'pspnet', 'deeplabv2'],
                        default=model_choices[model_index])
...
def main(args):
    ...
    # best_model = model
    # state_dict = torch.load("/root/ST-PlusPlus-master/outdir/models/COVERAGE_AUG/1-8/pspnet_resnet50_best.pth")
...
def init_basic_elems(args):
    ...
    if args.model == 'deeplabv2':
        ...
        state_dict = torch.load('pretrained/deeplabv2_resnet101_coco_pretrained.pth')
        new_state_dict = dict()
    ...
    for epoch in range(args.epochs):
        ...
        if mIOU >= previous_best:
            ...
            if MODE == 'train':
                ...
                torch.save(model.module.state_dict(),
                       os.path.join(args.save_path, '%s_%s_best.pth' % (args.model, args.backbone)))
            sout("new_best_model")
            best_model = deepcopy(model)
            drop = 0
        else:
...
if __name__ == '__main__':
    ...
    sout(model_choices[model_index])
    sout("")
    sout(args)
